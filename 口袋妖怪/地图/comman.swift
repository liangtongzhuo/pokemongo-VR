...
import UIKit
  ...
  ///判断是否是6p
  let iPhone6Push:Bool = UIScreen.mainScreen().bounds.width == 414 ?  true : false
  ///判断是否是6
  let iPhone6:Bool = UIScreen.mainScreen().bounds.width == 375 ?  true : false
  ///判断是否是5s 5e 4s
  let iPhone5s5e4s = iPhone6 != true && iPhone6Push != true ? true : false
   ///判断是否是4s
  let iPhone4s = UIScreen.mainScreen().bounds.height == 480 ? true : false
